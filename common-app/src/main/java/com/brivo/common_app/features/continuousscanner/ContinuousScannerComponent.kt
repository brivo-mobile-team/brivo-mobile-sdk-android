...
/**
 * Continuous Scanner unlock button with dynamic icon and device label
 */
@Composable
fun ContinuousScannerButton(
    viewState: ContinuousScannerViewState,
    onUnlockClick: () -> Unit,
    ...
    Column(
        ...
        ContinuousScannerButtonContent(
            modifier = Modifier,
            viewState = viewState,
...
@Composable
private fun ContinuousScannerButtonContent(
    modifier: Modifier,
    viewState: ContinuousScannerViewState,
    onUnlockClick: () -> Unit
) {
        ...
        Row(
            ...
            if (viewState.unlockStatus == UnlockStatus.InProgress) {
                ...
                Icon(
                    ...
                    contentDescription = stringResource(id = R.string.continuous_scanner_title),
                    modifier = Modifier.size(24.dp),
            ...
            val defaultDeviceName = stringResource(R.string.continuous_scanner_default_device_name)
            val deviceName = viewState.nearestDeviceName.ifEmpty { defaultDeviceName }

            val buttonText = when {
                viewState.unlockStatus is UnlockStatus.Success ->
                    stringResource(R.string.continuous_scanner_unlocked_device, deviceName)

                viewState.unlockStatus is UnlockStatus.Error ->
                    stringResource(R.string.continuous_scanner_failed_to_unlock)

                viewState.unlockStatus is UnlockStatus.InProgress ->
                    stringResource(R.string.continuous_scanner_unlocking_device, deviceName)

                viewState.nearestDeviceName.isEmpty() ->
                    stringResource(R.string.continuous_scanner_no_devices)

                else ->
                    stringResource(R.string.continuous_scanner_unlock_device, deviceName)
            }
